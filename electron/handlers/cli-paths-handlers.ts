 ...
 */
async function detectCLIPaths(): Promise<{ claude: string; codex: string; gemini: string; pi: string; gws: string; gcloud: string; gh: string; node: string }> {
  const homeDir = os.homedir();
  const paths = { claude: '', codex: '', gemini: '', pi: '', gws: '', gcloud: '', gh: '', node: '' };
...
export function registerCLIPathsHandlers(deps: CLIPathsHandlerDependencies): void {
  ...
  ipcMain.handle('cliPaths:get', async () => {
    ...
    return settings.cliPaths || { claude: '', codex: '', gemini: '', pi: '', gws: '', gcloud: '', gh: '', node: '', additionalPaths: [] };
  });
