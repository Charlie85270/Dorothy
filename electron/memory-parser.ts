...
export type ObservationType = 'tool_use' | 'message' | 'file_edit' | 'command' | 'decision';
...
class TerminalBuffer {
  ...
  private findSequenceEnd(input: string, start: number): number {
 ...
 */
export function renderTerminalOutput(chunks: string[]): string {
  const buffer = new TerminalBuffer();
...
function isCleanText(text: string): boolean {
  ...
  if (total > 0 && alphaNum < total * 0.4) return false;
  return true;
}
 ...
 */
export function parseOutputChunk(output: string): ParsedObservation[] {
  ...
  for (const [toolType, pattern] of Object.entries(TOOL_PATTERNS)) {
    ...
    if (matches && matches[1]) {
      ...
      let obsType: ObservationType = 'tool_use';
 ...
 */
export function parseAccumulatedOutput(outputChunks: string[]): ParsedObservation[] {
 ...
 */
export function isSignificantObservation(obs: ParsedObservation): boolean {
  ...
  if (obs.type === 'message') {
    const meta = obs.metadata as { messageType?: string } | undefined;
 ...
 */
export function filterSignificantObservations(observations: ParsedObservation[]): ParsedObservation[] {
 ...
 */
export function summarizeObservations(observations: ParsedObservation[]): string {
  const grouped = {
    file_edit: [] as string[],
    command: [] as string[],
    decision: [] as string[],
