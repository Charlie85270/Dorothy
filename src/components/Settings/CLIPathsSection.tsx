...
export const CLIPathsSection = ({ appSettings, onSaveAppSettings }: CLIPathsSectionProps) => {
  ...
  const [localPaths, setLocalPaths] = useState<CLIPaths>(
    appSettings.cliPaths || { claude: '', codex: '', gh: '', node: '', additionalPaths: [] }
  );

  useEffect(() => {
    setLocalPaths(appSettings.cliPaths || { claude: '', codex: '', gh: '', node: '', additionalPaths: [] });
  ...
  const hasChanges = JSON.stringify(localPaths) !== JSON.stringify(appSettings.cliPaths || { claude: '', codex: '', gh: '', node: '', additionalPaths: [] });
    ...
    <div className="space-y-6">
        ...
        {detectedPaths && (
          <div className="p-3 bg-green-500/10 border border-green-500/30 text-green-400 text-sm">
            ...
            <ul className="text-xs space-y-1 ml-6">
              ...
              {!detectedPaths.claude && !detectedPaths.codex && !detectedPaths.gh && !detectedPaths.node && (
      ...
      <div className="border border-border bg-card p-6">
        ...
        {renderPathInput(
          ...
          'codex',
          '/usr/local/bin/codex or ~/.npm/bin/codex'
        )}
